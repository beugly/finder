...
public protocol FinderSequence{
    ...
    /// Inserts the given element into the sequence unconditionally.
    /// - Parameter newElement: An element to insert into the sequence
    /// - Returns: element equal to `newMember` if the sequence already contained
    ...
    /// - Returns: the existing associated element of 'vertex' if 'vertex' already in the sequence; otherwise, 'nil'.
