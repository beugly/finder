...
public protocol FinderSourceType {
    ...
    ///Return neighbors (around point)
    func getNeighbors(around: Point) -> [Point]
}


public protocol FinderProtocol2 {
    
    associatedtype Heap: FinderHeapType;
    
    associatedtype Source: FinderSourceType;
    
    
    func heapGenerate() -> Heap
}
